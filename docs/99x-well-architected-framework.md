...
It's a pressing concern for most of the organizations when it comes to the costing of their infrastructure. You need to be strategic when you come up with the best
cost viable infrastructure solution, yet achieving the application requirements. Keep in mind that most of competitive features that the infrastructure providers offer might not always be fitting to the current state of your business.

ICOQ1 : Do you have a clear understanding on SLAs?

- [ ] Try not to make assumptions
- [ ] Get involved with the development team and do test evaluations, on a simulated environment (Staging)

ICOQ2 : Have you identified the minimal infrastructure needs that can serve the required level of performance and security?

- [ ] Study the Non functional requirements thoroughly
- [ ] Compare the numbers with the latest benchmarks when choosing your infrastructure options against pricing
- [ ] Try not to be too optimistic. Things may go wrong and buffer for it.

ICOQ3 : Have you analysed the cost against the benefit, on your chosen infrastructure services and tools?

- [ ] List all the benefits and risks of the chosen alternatives
- [ ] Have direct discussions with stake holders to convince them on critical areas that cannot be compromised with cost
- [ ] Maintain your decision log

ICOQ4 : If it is cloud, have you thought about automated resource planing features of your cloud providers? 

- [ ] Identify your thresholds on scaling
- [ ] Forecast future cost over growth

##### Monitoring

Once your application is in production for the first time or else a new version is being published, you have to capture the insights on it's behaviour in terms of security and performance. This information will play a major role in future decision makings and help to see the real picture of the assumptions that we have made in each past. Be it cloud or on premises, there are so many options to choose to implement a robust monitoring system.

IMQ1 : Have you spend some time to figure out the monitoring needs of your application and the business perspective?

- [ ] Carfully assess your existing infrastructure, tooling, resources
- [ ] Proper application monitoring will give insights on application feature usage. Check with dev team, to get to know about such requirements
- [ ] If there are mobile devices or IoT devices pay attention on them as these are always tied up with native issues like network drops.

IMQ2 : Have you identified the metrics that are cruical and non cruical?
...
IMQ3 : Do you know the logs created by the application and other general logs in the system?

- [ ] Work with the development team to identify all the logs that are generated by the application
- [ ] Tweak the application in order to support data capturing for the monitoring agents/tools
- [ ] Check the log levels are maintained properly which otherwise will cause unwanted resource usages and noises in monitoring tools

IMQ4 : Have you done a trial run at least for the custom alerts?

- [ ] Make sure you get the alerts when it happens for real
- [ ] Check whether the notification payload contains the necessary information like correlation ids, to track the incidents quickly
